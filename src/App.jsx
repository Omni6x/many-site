...
const copy = {
  en: {
    ...
    reserveText: "Leave your size and contact. The request will be copied, then Telegram opens. Paste the message and send it to confirm your place in the drop.",
    name: "Name",
    ...
    messageWant: "I want to reserve MANY logo tee for ~15€.",
  },
  uk: {
    drop: "Дроп 001",
    register: "Реєстрація",
    ...
    reserveText: "Залиш розмір і контакт. Заявка скопіюється, потім відкриється Telegram. Встав повідомлення та надішли його, щоб підтвердити місце у дропі.",
    name: "Імʼя",
    ...
    messageIntro: "MANY DROP 001 REGISTRATION",
    messageWant: "Хочу забронювати футболку MANY logo tee за ~15€.",
  },
};
      ...
      {["en", "uk"].map((item) => (
        <button
          ...
          className={`rounded-full px-3 py-1.5 text-xs font-black uppercase tracking-[0.16em] transition ${
            lang === item ? "bg-white text-black" : "text-zinc-500 hover:text-white"
          }`}
        >
          {item}
        </button>
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-30 bg-black/60 backdrop-blur-xl">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        <a href="#top" className="text-lg font-black uppercase tracking-[-0.06em]">
          MANY
        </a>
...
function TelegramButton({ children, variant = "light", className = "" }) {
  ...
  return (
    <button
      ...
      className={`inline-flex items-center justify-center gap-2 rounded-full border px-5 py-3 text-sm font-black uppercase tracking-[0.14em] transition hover:scale-[1.02] ${
        light
...
function TeeMockup({ t }) {
  return (
    <div className="relative mx-auto flex min-h-[430px] w-full max-w-md items-center justify-center rounded-[2rem] border border-white/10 bg-white/[0.03] p-6">
      <div className="absolute left-5 top-5 rounded-full border border-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-zinc-500">
        drop 001
      </div>
      <div className="absolute right-5 top-5 rounded-full border border-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.22em] text-zinc-500">
        {DROP_PRICE}
      </div>
      ...
      <div className="relative h-[350px] w-[270px]">
        ...
        <div className="absolute bottom-8 left-1/2 z-20 -translate-x-1/2 whitespace-nowrap rounded-full border border-black/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-black/45">
          {t.mockup}
        </div>
      </div>
    </div>
    ...
    <form onSubmit={registerDrop} className="rounded-[2rem] border border-white/10 bg-white/[0.035] p-5 sm:p-7">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block">
          ...
          <input
            ...
            className="w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm text-white outline-none transition placeholder:text-zinc-700 focus:border-white/40"
          />
        </label>
        ...
        <label className="block">
          ...
          <select
            ...
            className="w-full rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm text-white outline-none transition focus:border-white/40"
          >
            <option>XS</option>
            <option>S</option>
            <option>M</option>
      ...
      <label className="mt-3 block">
        ...
        <textarea
          ...
          rows={3}
          className="w-full resize-none rounded-2xl border border-white/10 bg-black px-4 py-3 text-sm text-white outline-none transition placeholder:text-zinc-700 focus:border-white/40"
        />
      </label>

      <button
        type="submit"
        className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-white px-5 py-4 text-sm font-black uppercase tracking-[0.14em] text-black transition hover:scale-[1.01] hover:bg-zinc-200"
      >
        {t.submit} <MessageCircle className="h-4 w-4" />
      </button>

      <p className="mt-4 text-xs leading-6 text-zinc-500">
        {copied ? t.copied : t.notCopied}
      </p>
    </form>
  );
}
    ...
    <main className="min-h-screen overflow-hidden bg-[#050505] text-white selection:bg-white selection:text-black">
      ...
      <section id="top" className="relative z-10 flex min-h-screen flex-col items-center justify-center px-4 pt-20 text-center">
        ...
        >
          ...
          <h1 className="text-[22vw] font-black uppercase leading-[0.75] tracking-[-0.13em] sm:text-9xl lg:text-[11rem]">
            MANY
          </h1>
          <p className="mt-6 text-xs font-black uppercase tracking-[0.28em] text-zinc-500">
            {t.heroLine}
          </p>
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            <a
              href="#drop"
              className="inline-flex items-center justify-center rounded-full bg-white px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-black transition hover:scale-[1.02] hover:bg-zinc-200"
            >
              {t.viewDrop}
            </a>
            <a
              href="#register"
              className="inline-flex items-center justify-center rounded-full border border-white/15 px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-white transition hover:bg-white/10"
            >
              {t.reserve}
            </a>
          </div>
          ...
          <div className="flex flex-col justify-between rounded-[2rem] border border-white/10 bg-white/[0.035] p-6 sm:p-8">
            <div>
              <div className="mb-6 flex flex-wrap gap-2">
                ...
                <span className="rounded-full border border-white/10 px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">{DROP_PRICE}</span>
              </div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <a
                href="#register"
                className="inline-flex items-center justify-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-black uppercase tracking-[0.14em] text-black transition hover:scale-[1.02] hover:bg-zinc-200"
              >
                {t.registerForDrop} <CheckCircle2 className="h-4 w-4" />
              </a>
